...
SYNTHESIZE_SINGLETON_FOR_CLASS(JSGCDDispatcher);
...
- (void)dispatchAsync:(void (^)(void))block priority:(dispatch_queue_priority_t)priority {
  dispatch_async(dispatch_get_global_queue(priority, 0), block);
}

- (void)dispatchAsync:(void (^)(void))block {
  [self dispatchAsync:block priority:DISPATCH_QUEUE_PRIORITY_DEFAULT];
}
...
- (void)dispatchBackgroundTask:(void (^)(UIBackgroundTaskIdentifier identifier))block
                      priority:(dispatch_queue_priority_t)priority {
    UIBackgroundTaskIdentifier bgTask = UIBackgroundTaskInvalid;
    
    bgTask = [self.application beginBackgroundTaskWithExpirationHandler:^{
  ...
  void(^gcdBlock)(void) = ^{
    ...
    @catch (NSException *exception) {
      DLog(@"Exception thrown in backgrond task: %@", exception);
    }
    @finally {
      [self.application endBackgroundTask:identifier];
    }
  };
  [self dispatchAsync:gcdBlock priority:priority];
}
#endif
