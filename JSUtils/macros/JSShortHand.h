...
#define DLog( s, ... ) \
[JSLogUtils log:[[NSString stringWithUTF8String:__FILE__] lastPathComponent] \
           line:__LINE__ \
        content:[NSString stringWithFormat:(s), ##__VA_ARGS__]];
...
#define JS_Stub_Throw(_name, _reason, ...) \
@throw [NSException exceptionWithName:(_name) \
reason:[[NSString stringWithFormat:@"%@: ",self.class]\
stringByAppendingFormat:(_reason), ##__VA_ARGS__] \
userInfo:nil]
...
#define JS_IsBetweenRange(value, range) ((value) <= (range.length) && (value) >= (range.location))
