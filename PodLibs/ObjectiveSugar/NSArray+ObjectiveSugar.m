...
-(id)inject:(id)memo block:(id (^)(id memo, id element))block{
    __block id blockMemo = memo;
    [self each:^(id object) {
        blockMemo = block(blockMemo, object);
    }];
    
    return blockMemo;
}

-(NSArray*)divide:(NSUInteger)subElementsCount{
    if (subElementsCount <= 0 || self.count <= subElementsCount) return @[self];
    
    NSUInteger maxCount = self.count;
    NSUInteger startIndex = 0;
    NSUInteger takeLength = subElementsCount;
    
    NSMutableArray *sections = [NSMutableArray arrayWithCapacity:maxCount / subElementsCount + 1];
    do {
        NSRange sub = NSMakeRange(startIndex, takeLength);
        [sections addObject:[self subarrayWithRange:sub]];
        
        NSUInteger rest = maxCount - startIndex - takeLength;
        
        startIndex += takeLength;
        takeLength = rest < takeLength ? rest : takeLength;
