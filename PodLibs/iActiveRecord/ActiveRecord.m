...
- (id)belongsTo:(NSString *)aClassName {
    ...
    [fetcher whereField:@"id"
           equalToValue:rec_id];
    return [[fetcher fetchRecords] firstObject];
}
...
- (void)removeRecord:(ActiveRecord *)aRecord through:(NSString *)aClassName
{
    ...
    ActiveRecord *record = [[fetcher fetchRecords] firstObject];
    [record dropRecord];
    [fetcher release];
}
...
+ (void)dropAllRecords {
    NSString *deleteAll = [NSString stringWithFormat:@"delete from %@", [[self recordName] quotedString]];
    [[ARDatabaseManager sharedInstance] executeSqlQuery:[deleteAll UTF8String]];
}
...
+ (ARColumn *)columnNamed:(NSString *)aColumnName {
    ...
    return [[columns filteredArrayUsingPredicate:predicate] firstObject];
}
